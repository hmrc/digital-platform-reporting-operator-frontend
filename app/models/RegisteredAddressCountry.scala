...
object RegisteredAddressCountry extends Enumerable.Implicits {
  ...
  def options(implicit messages: Messages): Seq[RadioItem] = values.zipWithIndex.map {
    case (value, index) =>
      RadioItem(
        content = Text(messages(s"registeredInUk.${value.toString}")),
        value   = Some(value.toString),
        id      = Some(s"value_$index"),
        hint    = value.toString match {
          case RegisteredAddressCountry.Uk.toString => Some(HintViewModel(Text(messages(s"registeredInUk.${value.toString}.hint"))))
          case _ => None
        }
      )
  }
