...
case class UpdatePlatformOperatorRequest(subscriptionId: String,
                                         operatorId: String,
                                         operatorName: String,
...
object UpdatePlatformOperatorRequest {
  implicit lazy val writes: OWrites[UpdatePlatformOperatorRequest] = Json.writes

  def build(answers: UserAnswers, dprsId: String, operatorId: String): EitherNec[Query, UpdatePlatformOperatorRequest] =
    (
  ...
  private[requests] def getUkTinDetails(answers: UserAnswers): EitherNec[Query, Seq[TinDetails]] =
    answers.getEither(UkTaxIdentifiersPage).flatMap(identifiers => identifiers.toSeq.parTraverse(getUkTin(_, answers)))
