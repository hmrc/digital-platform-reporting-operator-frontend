  ...
  def onPageLoad(operatorId: String): Action[AnyContent] = (identify andThen getData(Some(operatorId)) andThen requireData) { implicit request =>
    getAnswer(TaxResidencyCountryPage) { country =>
      ...
      val preparedForm = request.userAnswers.get(InternationalTaxIdentifierPage) match {
       ...
       Ok(view(preparedForm, operatorId, country))
    }
  }
  ...
  def onSubmit(operatorId: String): Action[AnyContent] = (identify andThen getData(Some(operatorId)) andThen requireData).async { implicit request =>
    getAnswerAsync(TaxResidencyCountryPage) { country =>
      ...
      form.bindFromRequest().fold(
        formWithErrors =>
          Future.successful(BadRequest(view(formWithErrors, operatorId, country))),

        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(InternationalTaxIdentifierPage, value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(InternationalTaxIdentifierPage.nextPage(operatorId, updatedAnswers))
      )
    }
  }
}
