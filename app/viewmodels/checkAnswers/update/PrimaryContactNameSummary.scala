...
object PrimaryContactNameSummary  {
  ...
  def row(operatorId: String, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    for {
      answer       <- answers.get(PrimaryContactNamePage)
      businessName <- answers.get(BusinessNamePage)
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.PrimaryContactNameController.onPageLoad(operatorId).url)
              .withVisuallyHiddenText(messages("primaryContactName.change.hidden", businessName))
          )
        )
    }
}
