...
object HasTradingNameSummary  {
  ...
  def row(operatorId: String, answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    for {
      answer       <- answers.get(HasTradingNamePage)
      businessName <- answers.get(BusinessNamePage)
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.HasTradingNameController.onPageLoad(operatorId).url)
              .withVisuallyHiddenText(messages("hasTradingName.change.hidden", businessName))
          )
        )
    }
}
