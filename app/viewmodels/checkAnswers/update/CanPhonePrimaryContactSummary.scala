...
import pages.update.{CanPhonePrimaryContactPage, PrimaryContactNamePage}
    ...
    for {
      ...
      contactName <- answers.get(PrimaryContactNamePage)
        ...
        SummaryListRowViewModel(
          key     = messages("canPhonePrimaryContact.checkYourAnswersLabel", contactName),
          value   = ValueViewModel(value),
          actions = Seq(
            ActionItemViewModel("site.change", routes.CanPhonePrimaryContactController.onPageLoad(operatorId).url)
