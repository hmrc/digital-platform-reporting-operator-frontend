...
final case class PlatformOperatorSummary(operatorId: String, operatorName: String) {
  ...
  def summaryListRow(implicit messages: Messages): SummaryListRow =
    SummaryListRowViewModel(
      ...
      actions = Seq(
        ActionItemViewModel(Text(messages("site.view")), routes.PlatformOperatorController.onPageLoad(operatorId).url)
...
object PlatformOperatorSummary {
  ...
  def apply(operator: PlatformOperator): PlatformOperatorSummary =
    PlatformOperatorSummary(operator.operatorId, operator.operatorName)
}
