...
import models.addressLookup.AddressLookupModel
...
object ConfirmedAddressHttpParser {
  ...
  type ConfirmedAddressResponse = Either[ErrorResponse, AddressLookupModel]
  ...
  implicit object ConfirmedAddressReads extends HttpReads[ConfirmedAddressResponse] {
    ...
    def read(method: String, url: String, response: HttpResponse): ConfirmedAddressResponse =
      response.status match {
        case OK        =>
          response.json.validate[AddressLookupModel](AddressLookupModel.responseReads) match {
