...
class SubscriptionConnector @Inject()(configuration: Configuration,
  ...
  def getSubscriptionInfo(implicit hc: HeaderCarrier): Future[SubscriptionInfo] =
    httpClient.get(url"$digitalPlatformReporting/digital-platform-reporting/subscribe")
      ...
      .flatMap { response =>
        response.status match {
          ...
          case status => Future.failed(GetSubscriptionInfoFailure(status))
        }
      }
}

object SubscriptionConnector {
  final case class GetSubscriptionInfoFailure(statusCode: Int) extends Throwable {
