...
class SubscriptionConnectorSpec extends AnyFreeSpec
  ...
  ".getSubscription" - {
    "must return subscription info when the server returns OK" in {
      ...
      val result = connector.getSubscriptionInfo(hc).futureValue
    ...
    "must fail when the server returns an error" in {
      ...
      val result = connector.getSubscriptionInfo(hc).failed.futureValue
      result mustBe a[GetSubscriptionInfoFailure]

      val getSubscriptionFailure = result.asInstanceOf[GetSubscriptionInfoFailure]
