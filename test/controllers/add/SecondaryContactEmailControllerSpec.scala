...
import pages.add.{BusinessNamePage, SecondaryContactEmailPage, SecondaryContactNamePage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.add.SecondaryContactEmailView
...
class SecondaryContactEmailControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val secondaryContactEmailRoute = routes.SecondaryContactEmailController.onPageLoad(NormalMode).url
  ...
  "SecondaryContactEmail Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("foo@example.com"), NormalMode, businessName, contactName)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual SecondaryContactEmailPage.nextPage(NormalMode, emptyUserAnswers).url
      }
    }
