...
import views.html.add.TradingNameView
...
class TradingNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val tradingNameRoute: String = routes.TradingNameController.onPageLoad(NormalMode).url
  ...
  "TradingName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("answer"), NormalMode, businessName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual TradingNamePage.nextPage(NormalMode, emptyUserAnswers).url
      }
    }
