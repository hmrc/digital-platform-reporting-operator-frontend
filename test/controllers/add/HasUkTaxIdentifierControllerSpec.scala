...
import forms.add.HasUkTaxIdentifierFormProvider
import models.NormalMode
import org.mockito.ArgumentMatchers.any
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import pages.add.{BusinessNamePage, HasUkTaxIdentifierPage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.add.HasUkTaxIdentifierView
...
class HasUkTaxIdentifierControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val hasUkTaxIdentifierRoute = routes.HasUkTaxIdentifierController.onPageLoad(NormalMode).url
  ...
  "HasUkTaxIdentifier Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = baseAnswers.set(HasUkTaxIdentifierPage, true).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual HasUkTaxIdentifierPage.nextPage(NormalMode, expectedAnswers).url
      }
    }
