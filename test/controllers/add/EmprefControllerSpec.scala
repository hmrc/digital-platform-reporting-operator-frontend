...
import views.html.add.EmprefView
...
class EmprefControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val emprefRoute: String = routes.EmprefController.onPageLoad(NormalMode).url
  ...
  "Empref Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("123/AB456"), NormalMode, businessName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual EmprefPage.nextPage(NormalMode, emptyUserAnswers).url
      }
    }
