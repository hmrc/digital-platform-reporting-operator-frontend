...
import pages.add.{BusinessNamePage, CanPhonePrimaryContactPage, PrimaryContactNamePage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.add.CanPhonePrimaryContactView
  ...
  "CanPhonePrimaryContact Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), NormalMode, businessName, contactName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = baseAnswers.set(CanPhonePrimaryContactPage, true).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual CanPhonePrimaryContactPage.nextPage(NormalMode, expectedAnswers).url
      }
    }
