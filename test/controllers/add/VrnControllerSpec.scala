...
import pages.add.{BusinessNamePage, VrnPage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.add.VrnView
...
class VrnControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val vrnRoute: String = routes.VrnController.onPageLoad(NormalMode).url
  ...
  "Vrn Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("GB123456789"), NormalMode, businessName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual VrnPage.nextPage(NormalMode, emptyUserAnswers).url
      }
    }
