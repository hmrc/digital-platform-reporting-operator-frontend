  ...
  "PrimaryContactEmail Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("foo@example.com"), NormalMode, businessName, contactName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual PrimaryContactEmailPage.nextPage(NormalMode, emptyUserAnswers).url
      }
    }
