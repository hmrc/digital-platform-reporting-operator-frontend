...
import models.operator.{AddressDetails, ContactDetails, PlatformOperatorData}
...
class SelectPlatformOperatorToViewControllerSpec extends SpecBase with MockitoSugar with BeforeAndAfterEach {
  ...
  private lazy val selectPlatformOperatorRoute = routes.SelectPlatformOperatorToViewController.onPageLoad.url
  ...
  private val formProvider = new SelectPlatformOperatorToViewFormProvider()
  ...
  "Select Platform Operator Controller" - {
    ...
    "must save platform operator details and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual SelectPlatformOperatorToViewPage.nextPage(NormalMode, operator1.operatorId, emptyUserAnswers).url
