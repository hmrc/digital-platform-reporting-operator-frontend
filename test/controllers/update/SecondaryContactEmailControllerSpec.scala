...
import pages.update.{BusinessNamePage, SecondaryContactEmailPage, SecondaryContactNamePage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.update.SecondaryContactEmailView

import scala.concurrent.Future

class SecondaryContactEmailControllerSpec extends SpecBase with MockitoSugar {

  private val formProvider = new SecondaryContactEmailFormProvider()
  ...
  private lazy val secondaryContactEmailRoute = routes.SecondaryContactEmailController.onPageLoad(operatorId).url

  "SecondaryContactEmail Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, secondaryContactEmailRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("foo@example.com"), operatorId, businessName, contactName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual SecondaryContactEmailPage.nextPage(operatorId, emptyUserAnswers).url
      }
    }
