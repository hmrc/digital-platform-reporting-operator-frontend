...
class CanPhonePrimaryContactControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val canPhonePrimaryContactRoute = routes.CanPhonePrimaryContactController.onPageLoad(operatorId).url
  ...
  "CanPhonePrimaryContact Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), operatorId, businessName, contactName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual CanPhonePrimaryContactPage.nextPage(operatorId, expectedAnswers).url
      }
    }
