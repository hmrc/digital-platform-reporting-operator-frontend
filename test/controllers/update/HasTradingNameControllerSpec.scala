...
import pages.update.{BusinessNamePage, HasTradingNamePage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.update.HasTradingNameView
...
class HasTradingNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val hasTradingNameRoute = routes.HasTradingNameController.onPageLoad(operatorId).url
  ...
  "HasTradingName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = baseAnswers.set(HasTradingNamePage, true).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), operatorId, businessName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = baseAnswers.set(HasTradingNamePage, true).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual HasTradingNamePage.nextPage(operatorId, expectedAnswers).url
      }
    }
