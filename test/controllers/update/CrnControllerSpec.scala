...
import forms.update.CrnFormProvider
import org.mockito.ArgumentMatchers.any
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import pages.update.{BusinessNamePage, CrnPage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.update.CrnView

import scala.concurrent.Future

class CrnControllerSpec extends SpecBase with MockitoSugar {

  private val formProvider = new CrnFormProvider()
  ...
  lazy val crnRoute = routes.CrnController.onPageLoad(operatorId).url
  ...
  "Crn Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("AB123456"), operatorId, businessName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual CrnPage.nextPage(operatorId, emptyUserAnswers).url
      }
    }
