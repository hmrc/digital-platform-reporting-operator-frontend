...
import pages.update.{BusinessNamePage, PrimaryContactNamePage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.update.PrimaryContactNameView
...
class PrimaryContactNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val primaryContactNameRoute = routes.PrimaryContactNameController.onPageLoad(operatorId).url
  ...
  "PrimaryContactName Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("answer"), operatorId, businessName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual PrimaryContactNamePage.nextPage(operatorId, emptyUserAnswers).url
      }
    }
