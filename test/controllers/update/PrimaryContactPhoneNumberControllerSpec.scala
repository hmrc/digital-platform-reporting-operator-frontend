...
import forms.update.PrimaryContactPhoneNumberFormProvider
...
class PrimaryContactPhoneNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val primaryContactPhoneNumberRoute = routes.PrimaryContactPhoneNumberController.onPageLoad(operatorId).url
  ...
  "PrimaryContactPhoneNumber Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual PrimaryContactPhoneNumberPage.nextPage(operatorId, emptyUserAnswers).url
      }
    }
