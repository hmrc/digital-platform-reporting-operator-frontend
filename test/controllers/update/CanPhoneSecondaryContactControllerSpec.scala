...
import forms.update.CanPhoneSecondaryContactFormProvider
...
class CanPhoneSecondaryContactControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val canPhoneSecondaryContactRoute = routes.CanPhoneSecondaryContactController.onPageLoad(operatorId).url
  ...
  "CanPhoneSecondaryContact Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual CanPhoneSecondaryContactPage.nextPage(operatorId, expectedAnswers).url
      }
    }
