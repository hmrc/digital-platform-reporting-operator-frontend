...
class ChrnControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val chrnRoute = routes.ChrnController.onPageLoad(operatorId).url
  ...
  "Chrn Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("X00000"), operatorId, businessName)(using request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual ChrnPage.nextPage(operatorId, emptyUserAnswers).url
      }
    }
