...
import forms.update.EmprefFormProvider
import org.mockito.ArgumentMatchers.any
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import pages.update.{BusinessNamePage, EmprefPage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.update.EmprefView

import scala.concurrent.Future

class EmprefControllerSpec extends SpecBase with MockitoSugar {

  private val formProvider = new EmprefFormProvider()
  ...
  lazy val emprefRoute = routes.EmprefController.onPageLoad(operatorId).url
  ...
  "Empref Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill("123/AB456"), operatorId, businessName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual EmprefPage.nextPage(operatorId, emptyUserAnswers).url
      }
    }
