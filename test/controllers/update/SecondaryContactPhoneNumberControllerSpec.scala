...
class SecondaryContactPhoneNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val secondaryContactPhoneNumberRoute = routes.SecondaryContactPhoneNumberController.onPageLoad(operatorId).url
  ...
  "SecondaryContactPhoneNumber Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual SecondaryContactPhoneNumberPage.nextPage(operatorId, emptyUserAnswers).url
      }
    }
