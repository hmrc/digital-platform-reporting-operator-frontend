...
import forms.update.TaxResidencyCountryFormProvider
import models.Country
import org.mockito.ArgumentMatchers.any
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import pages.update.{BusinessNamePage, TaxResidencyCountryPage}
import play.api.inject.bind
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.update.TaxResidencyCountryView
...
class TaxResidencyCountryControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val taxResidencyCountryRoute = routes.TaxResidencyCountryController.onPageLoad(operatorId).url
  ...
  "TaxResidencyCountry Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(country), operatorId, businessName)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual TaxResidencyCountryPage.nextPage(operatorId, emptyUserAnswers).url
      }
    }
