...
class EmprefPageSpec
  ...
  ".nextPage" - {
    ...
    "must go to CHRN when CHRN is selected and has not been answered" in {
      ...
      EmprefPage.nextPage(operatorId, answers) mustEqual routes.ChrnController.onPageLoad(operatorId)
    }
    ...
    "must go to Check Answers" - {
      ...
      "when all selected options have been answered" in {
        ...
        forAll(identifierGen) { identifiers =>
          ...
          EmprefPage.nextPage(operatorId, answers) mustEqual routes.CheckYourAnswersController.onPageLoad(operatorId)
        }
      }
    }
  }
