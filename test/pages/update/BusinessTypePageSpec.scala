...
class BusinessTypePageSpec extends AnyFreeSpec with Matchers with TryValues with OptionValues {
  ...
  ".nextPage" - {
    ...
    "must go to Check Answers when UTR has been answered" in {
      ...
      BusinessTypePage.nextPage(operatorId, answers) mustEqual routes.CheckYourAnswersController.onPageLoad(operatorId)
    }

    "must go to UTR when UTR has not been answered" in {

      BusinessTypePage.nextPage(operatorId, emptyAnswers) mustEqual routes.UtrController.onPageLoad(operatorId)
    }
  }
}