...
class CreatePlatformOperatorRequestSpec extends AnyFreeSpec with Matchers
  ...
  private val underTest = CreatePlatformOperatorRequest
  ...
  ".build" - {
    "must create correct CreatePlatformOperatorRequest when all data provided" in {
      ...
      underTest.build(answers, "some-dprs-id") mustBe Right(CreatePlatformOperatorRequest(
    ...
    "must return a list of errors when not all data provided" in {
      ...
      val result = underTest.build(answers, "any-dprs-id")
      result.left.value.toChain.toList must contain theSameElementsAs Seq(
        BusinessNamePage,
        HasTradingNamePage,
